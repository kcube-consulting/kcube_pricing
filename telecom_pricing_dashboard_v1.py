...
def main():
    ...
    with tab4:
        ...
        # Create animated projection chart
        fig = make_subplots(specs=[[{"secondary_y": True}]])
        
        # Add cumulative cost traces
        fig.add_trace(
            go.Scatter(
                x=years,
        ...
        st.markdown("### Year-by-Year Cost Breakdown")
        year_data = {
            "Year": years,
            "Fixed Annual Cost": fixed_costs,
            "Fixed Cumulative": np.cumsum(fixed_costs),
            "PayG Annual Cost": payg_costs,
            "PayG Cumulative": np.cumsum(payg_costs),
            "Savings": np.cumsum(payg_costs) - np.cumsum(fixed_costs)
        }
        
        year_df = pd.DataFrame(year_data)
        st.dataframe(
            year_df.style.format({
                "Fixed Annual Cost": "${:,.2f}",
                "Fixed Cumulative": "${:,.2f}",
                "PayG Annual Cost": "${:,.2f}",
                "PayG Cumulative": "${:,.2f}",
                "Savings": "${:,.2f}"
        ...
        # Add download button for multi-year data
        csv = year_df.to_csv(index=False).encode('utf-8')
        st.download_button(
            label="Download Multi-Year Data",
            data=csv,
            file_name=f"kcube_{analysis_years}_year_projection.csv",
            mime="text/csv"
        )
